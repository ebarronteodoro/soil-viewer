...
const Scene = forwardRef(
  ({ activeMeshIndex, handleClick, object, isOpened }, ref) => {
    ...
    const animateZoom = delta => {
      ...
      const zoomAnimate = () => {
        ...
        const maxYLimit =
          zoomDistance.current >= 0.7 ? 3.8 : maxYLimitBase * 0.9
        if (
        ...
        if (
          Math.abs(targetZoomDistance.current - zoomDistance.current) > 0.01
        ) {
    ...
    useEffect(() => {
      ...
      const tryUpdateTargetAngle = () => {
        if (cameraRef.current) {
          updateTargetAngle(0.85)
        } else {
          setTimeout(tryUpdateTargetAngle, 100)
        }
      }

      tryUpdateTargetAngle()
  ...
  return (
    <div className='viewport-container'>
      ...
      {showContent && (
        <>
          <div
            className={`soil-info ${applyTransition ? 'show' : ''} ${
              isOpened && 'hidden'
            }`}
          >
          ...
          <div
            className={`floors-info ${selectedFloor && 'active'} ${
              isOpened && 'hidden'
            }`}
          >
            ...
            <div className='floor-details'>
              <span className='floor-capacity'>
                Departamentos:{' '}
                {departamentos[`planta_${selectedFloor}`]
                  ? departamentos[`planta_${selectedFloor}`].length
                  : 0}
              </span>
